	...
	<div class="panel-body">
		<div class="table-responsive" id="table">
			<table class="table table-condensed table-hover">
				...
				<tbody>
					<c:forEach items="${ t_dto }" var="dto">

						<tr>
							<td class="text-center">${dto.t_code }</td>
							<td class="text-center">${dto.t_name }</td>
							<td class="text-center">${dto.t_pay }</td>
							...
							<td class="text-center"><secu:authorize
									...
									<input type="button" class="btn btn-warning btn-sm" value="수정"
										onclick="window.location.href='treatment_update?t_code=${dto.t_code }'">
								</secu:authorize></td>
							<td class="text-center"><secu:authorize
									access="hasAnyRole('ROLE_patient_page2_master','ROLE_patient_page2_anything')">
									<input type="button" class="btn btn-danger btn-sm" value="삭제"
										onclick="window.location.href='treatment_delete?t_code=${dto.t_code }'">
								</secu:authorize></td>
						</tr>
					</c:forEach>
					...
					<tr>
								...
								<nav>
									<ul class="pagination" id="pagination">
										<c:if test="${startPage > pageBlock }">
											<li><a href="treatment_admin?pageNum=${startPage-5}"
												aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
											</a></li>
										</c:if>

										<c:forEach var="i" begin="${startPage }" end="${endPage }">
											<li><a href="treatment_admin?pageNum=${i }"> ${i } <span
						...
						<td class="thick-line" id="insert_button" align="center"><secu:authorize
								...
								<input type="button" value="추가" class="btn btn-success btn-sm"
									onclick="window.location.href = 'treatment_insert'">
							</secu:authorize></td>
